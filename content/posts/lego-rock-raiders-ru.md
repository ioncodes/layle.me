...
Notice how in the video, the rock count doesn't decrease - normally, upon upgrading the base it should play an animation as well as subtract the appropriate amount of rocks. Without this feature, it's not possible to progress in the game. At the time I thought this is likely some obscure bug, perhaps a "normal" bug within the game itself, whoever, the bug would never cause any issues due to some incosistency between older versions of Windows and modern ones (as seen in multiple older titles).
...
That looks fairly straight forward, doesn't it? Let's summarize:
...
The rest of `WinMain` calls some more functions, but I wasn't interested in that yet - all I wanted is to bypass the message box (the call to `MessageBoxA` in the following screenshot) and progress into the game.

![](/images/lrr-ru/winmainrest.png)

During debugging I noticed that I wasn't able set normal breakpoints as the game would eventually break the stack and execute a bogus address. I thought that it's likely due to some sort of integrity check (bonus points if you were already able to spot it in the screenshot) but didn't bother with it since I could just use hardware breakpoints. For some context: Software breakpoints overwrite a byte at the target location and set it to `0xCC` which is the equivalent of `int3` to cause an exception.  
...
Let's what's happening at `0041F92D`:
...
Seems simple enough to just search for the same code in the Russian version, right? My prefered method is to actually export the to-be-searched game version as C decompilation and then search a string like `-= a2` (something that's simple enough to be universal) in the Russian version. That yielded the following code:
...
As we know a `ret` pops from the stck and jumps to that location. IDA isn't able to deduce this and ends up not displaying the rest of the code. I didn't do this personally, but you could technically patch out these sequences with NOP slides which would restore the decompilation like so:
...
By the way, 2nd chance to spot the integrity check on your own - sadly I was too tired at this point to notice myself. :')
...
004781FF | 46                       | inc esi                     
00478200 | 49                       | dec ecx                     
00478201 | 75 FA                    | jne legorr.4781FD           
00478203 | 5E                       | pop esi                     
00478204 | BA 1B824700              | mov edx,legorr.47821B       
00478209 | 52                       | push edx                    
0047820A | BA E4814700              | mov edx,legorr.4781E4       
0047820F | 2B42 04                  | sub eax,dword ptr ds:[edx+4]
00478212 | 290424                   | sub dword ptr ss:[esp],eax  
00478215 | 33C0                     | xor eax,eax                 
00478217 | 64:8900                  | mov dword ptr fs:[eax],eax  
0047821A | C3                       | ret                         
```

Reading the disassembly (I ommitted a few things) reveals a loop that increments from address A to B and adds the 32bit value located at said address to a counter. This counter is then subtracted by a known value and the result is then used to subtracted the value that `ESP` points at. `ESP` "just so happens" to point to the next function executed. Therefore, if the checksum (the counter) does not match the predetermined value, the subtraction will be non-zero, which in turn means the first 4 bytes of the next function are subtracted by the left over amount. Short pseudocode:
...
I built a new version of the DLL and passed it along for testing as I seemed to be able to at least pass the TNT tutorial now. So at this point, the base upgrade actually work! Wow!
...
Nothing here, right? Well, we learnt our lesson. Let's dive into the disassembly:
...
DWORD __cdecl docdcheck(CHAR driveletter, unsigned __int16 idk)
{
  ...
  *(_DWORD *)FileName = *(_DWORD *)f_drive;
  ...
  if ( FileA != (HANDLE)-1 )
  {
    // Grab table of contests from CD
...
This time I ran the tests myself as you can unlock all levels using the argument `-testlevels` and the last known crash (not the integrity one, the gameplay one that would trigger randomly after a while) would just involved idling around for a few minutes. I crashed again, but in my console window I was greeted with a new log line:
...
During the research we noticed that some other Russian verions have the same DLLs, it's currently unknown to us whether there is actually more lost/unfixed games and media and we're currently trying to get our hands on them along with test setups to see if they work on modern machines or whether they need fixes/cracks as well.

Huge thank you to The Research Realm for trying to preserve a very important part of history! It's very sad to think that one day, these games might just be completely lost and it's therefore very important to back everything up as early as possible and make compatibility patches. The Research Realm is currently lead by the founder [Klavvy/baraklava](https://www.youtube.com/@Klavvy) of [Manic Miners](https://manicminers.baraklava.com/), a modern remake of Lego Rock Raiders and a current employee at The Lego Group. Lego endorses the project as long as there is no financial incenctive (e.g. donations, etc).